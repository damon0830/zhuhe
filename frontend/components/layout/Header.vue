  ...
  <header class="fixed top-0 left-0 right-0 z-50 bg-cream-50/95 backdrop-blur-sm">
    <div class="max-w-7xl mx-auto px-6">
      <div class="flex justify-between items-center h-10 text-xs text-ink-300 border-b border-cream-200">
        ...
        <div class="flex gap-6">
          <NuxtLink v-if="user" to="/orders" class="hover:text-navy transition-colors">My Orders</NuxtLink>
    ...
    <div v-if="mobileOpen" class="md:hidden bg-cream-50 border-t border-cream-200">
      <div class="px-6 py-4 flex flex-col gap-4 text-navy">
        ...
        <NuxtLink v-if="user" to="/orders" @click="mobileOpen = false">My Orders</NuxtLink>
...
onMounted(async () => {
  if (user.value) { try { await fetchCart() } catch {} }
})
</script>
<style scoped>
.nav-link { @apply text-ink-300 text-sm tracking-widest uppercase font-body hover:text-navy transition-colors duration-300; }
.nav-link.router-link-exact-active { @apply text-navy; }
</style>
